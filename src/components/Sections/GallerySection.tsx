...
const GallerySection = () => {
  ...
  return <section id="gallery" className="section-padding bg-background section-divider">
      <div className="container-padding">
        ...
        <Tabs defaultValue="alle" className="w-full">
          <TabsList className="grid w-full grid-cols-4 lg:grid-cols-7 mb-8 card-clean h-auto p-2">
            {galleryCategories.map(category => <TabsTrigger key={category.key} value={category.key} className="data-[state=active]:bg-primary data-[state=active]:text-primary-foreground transition-all duration-300 hover:bg-primary/10 flex flex-col items-center p-4 gap-2 group">
                <category.icon className="w-5 h-5 transition-transform duration-300 group-hover:scale-110" />
                <span className="text-xs font-medium">{category.title}</span>
              </TabsTrigger>)}
          </TabsList>

          {galleryCategories.map(category => <TabsContent key={category.key} value={category.key}>
              <div className="grid grid-4 grid-responsive">
                {filterItems(category.key).map((item, index) => <div key={index} className="group relative overflow-hidden rounded-xl shadow-sm hover:shadow-lg transition-all duration-500 cursor-pointer" onClick={() => setSelectedImage(item)}>
                    <div className="relative group overflow-hidden rounded-xl shadow-lg hover:shadow-2xl">
                      <img src={item.image} alt={item.title} className="w-full h-64 object-cover transition-all duration-700 group-hover:scale-125 group-hover:brightness-125 group-hover:contrast-110 group-hover:saturate-110 filter hover:drop-shadow-2xl" style={{
                  ...
                  transition: 'all 0.7s cubic-bezier(0.25, 0.46, 0.45, 0.94)'
                      ...
                      {item.isBeforeAfter && <div className="absolute top-4 left-4 animate-pulse-soft">
        ...
        <div className="mt-16 text-center">
          <div className="card-feature max-w-2xl mx-auto">
            ...
            <Button onClick={() => window.open("https://wa.me/4917398573699?text=Hallo! Ich möchte gerne einen Termin vereinbaren.", '_blank')} className="btn-hero">
      ...
      {selectedImage && <Dialog open={!!selectedImage} onOpenChange={() => setSelectedImage(null)}>
          <DialogContent className="max-w-4xl max-h-[90vh] overflow-hidden p-0">
            ...
            <div className="px-6 pb-6">
              <img src={selectedImage.image} alt={selectedImage.title} className="w-full h-auto max-h-[60vh] object-contain rounded-lg mb-4" />
              <p className="text-foreground/70 text-lg leading-relaxed">{selectedImage.description}</p>
              {selectedImage.isBeforeAfter && <div className="mt-4 inline-flex items-center bg-accent/20 backdrop-blur-sm border border-accent/30 rounded-full px-4 py-2">
